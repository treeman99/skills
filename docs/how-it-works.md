...
| 번호 | 규칙 | 언제 |
|---|---|---|
| 1 | 완료 조건을 체크리스트로, 범위 밖 파일 금지 | 착수 전 |
| 2 | 태스크 유형별 규칙 (코드 작업이면 실패 테스트 먼저) | 일을 시작할 때 |
| 3 | 수정 제안 전 근본 원인 추적, 3회 실패 시 escalation | 버그를 만났을 때 |
| 4 | 검증 명령을 실제로 실행하고 출력을 읽는다 | 완료 주장 직전 |
| 5 | 질문은 사람이 아니라 `orca orchestration ask` | 판단이 갈릴 때 |
| 6 | 보고서에 검증 명령과 결과, 미통과면 `--outcome failed` | 마지막 |

2번이 유형에 따라 갈리는 자리다.

| 태스크 유형 | 2번에 들어가는 것 |
|---|---|
| 기능 구현, 리팩터링 | 실패 테스트 먼저 |
| 버그 수정 | 실패 테스트 먼저 + 원 증상 재현 테스트 |
| 리뷰 전용 | 파일을 고치지 않는다 |
| 조사 | 읽은 파일과 근거를 경로:줄로 남긴다 |
