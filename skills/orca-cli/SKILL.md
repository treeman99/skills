...
- 출처: `stablyai/orca` · `skills/orca-cli/SKILL.md` (커밋 `dac82f61`, 2026-09-23).
...
- 상류 재확인(2026-09-21, v1.4.206 릴리스): `0d23ea6e` → `76d87604`에서 상류 `skills/`는 변경이 0건이라 이 파일의 상류 원문은 그대로다. **상류 description에는 두 문구가 아직 그대로 있다** — `v1.4.206:skills/orca-cli/SKILL.md`의 frontmatter를 위 grep으로 세면 2건이라 이 수정은 계속 필요하다. 포크 쪽 근거도 유지된다: `enterprise/samsungds`의 `src/cli/specs`에 `path: ['skills', 'share']`가 0건이고, `config/fork-feature-ledger.json`에 `fork-orca-cli-skill-text`가 그대로 등재돼 있다. 이번 머지에서 이 파일은 충돌하지 않았고, 머지 뒤 frontmatter grep도 0이다.
- 상류 재확인(2026-09-23, v1.4.209 릴리스): v1.4.207·v1.4.209 태그의 `skills/` 트리 해시는 `76d87604`와 같아 릴리스된 원문은 그대로다. 상류 main `dac82f61`에서는 `9af6a3d798`(#22341)이 본문 끝 문단에 `runtime_access_denied` 안내 한 문장을 끼웠고, 상류 원문이라 그대로 받았다. 이 문단이 바로 뒤의 이 절과 붙어 있어 머지 충돌이 났고, 상류 문단을 받고 이 절을 유지해 풀었다. **상류 description에는 두 문구가 아직 그대로 있다** — `v1.4.209`와 `dac82f61`의 frontmatter를 위 grep으로 세면 둘 다 2건이다. 포크 쪽 근거도 유지된다: `enterprise/samsungds`(`v1.4.207-samsungds`)의 `src/cli/specs`에 `path: ['skills', 'share']`가 0건이다. 머지 뒤 이 파일은 상류와 두 문구만 다르고, frontmatter grep은 0이다.
