...
One more thing to check before concluding the prompt was lost: `worker-show --dispatch
<dispatch_id> --json`, field `result.terminalResource.ownershipState`. If it reads
`user_owned` with reason `user_takeover` and nobody typed in the worker pane, the prompt was
written — `worker-prompt-sent` in the diagnostic log is the receipt — and what you are
looking at is the takeover deleting the output archive. Fork builds before commit
`52eeafc5a0` raised that takeover from a mouse wheel over the split worker pane, because
opencode enables mouse tracking and the renderer counted its reports as user input. Turning
off the worker pane auto-split setting avoids it on those builds; do not restart the worker.
...
- 출처: `stablyai/orca` · `skills/orchestration/SKILL.md` (커밋 `dac82f61`, 2026-09-23). 본문의 큰 변경은 `bba68b1b`였고, 마지막 변경은 `9af6a3d798`의 `runtime_access_denied` 한 문장이다(아래 2026-09-23 항목). frontmatter `description`은 `12d744f2`(#21069)에서 바뀌었고, 이 스킬은 상류 값을 그대로 받았다(아래 2026-09-17 항목).
...
- 상류 재확인(2026-09-14, v1.4.201 릴리스): 확인 경로는 포크 체크아웃 `/Users/daegun/Workspace/orca`의 `upstream` 리모트 태그(`v1.4.201` = `2b19f21a`, `v1.4.202` = `b1f70853`)와 사내 태그 `v1.4.200-samsungds`의 `skill-guides/`·`src/`다. **삭제 조건에 걸린 커스터마이징은 없다.** ⓐ v1.4.200 → v1.4.201에서 서비스 가이드가 바뀐 곳은 `references/messaging-and-gates.md` 한 곳이다 — `@all`·`@opencode` 같은 그룹 주소가 보낸 쪽 Run의 live Dispatch로 한정되고, Run에 묶이지 않은 발신자는 거절된다(`4b1b7178ad`, `src/cli/specs/orchestration.ts` notes 2줄 추가). 이 스킬의 커스터마이징은 그룹 주소를 쓰지 않으므로 겹치지 않는다. v1.4.201 → v1.4.202는 `skill-guides/` 변경이 없다. ⓑ v1.4.201·v1.4.202 가이드에도 `task-title`·`display-name`·`legacy_ambiguous`·`.orca/artifacts`·opencode 스펙 전달 방식은 나오지 않고(`git grep` 0건), 커스터마이징이 쓰는 `--task-title`·`--display-name`·`--report-path`·`--files-modified`·`--timeout-ms`와 `terminalResource`·`ownershipState`는 `src/`에 그대로 있다. 제목 파생(`orchestration-task-display.ts`)·릴리스 보존(`worker-terminal-release.ts`)·프리앰블(`preamble.ts`) 파일은 v1.4.201에서 변경이 없다. ⓒ opencode 절이 기대는 포크 전용 코드(`promptDeliveryMode`, `waitForAgentComposerReady`, `worker-pane-anchor-terminal.ts`, `terminal-typed-user-input.ts`)는 상류 v1.4.201에 여전히 없다 — 사내 빌드가 v1.4.201을 머지할 때 포크 쪽에서 유지돼야 하는 코드이고, 이 스킬이 바꿀 것은 없다. 포크는 아직 v1.4.201을 머지하지 않았다(2026-09-14 fetch 기준 최신 태그 `v1.4.200-samsungds`, `origin`과 동일). ⓓ 아래 추가 2건이 언급한 포크 가이드의 `Work Artifacts` 절(`f1c3963d`)은 `v1.4.198-samsungds`까지 있었고, 상류 v1.4.199 머지(`c0c421d6d9`)에서 빠졌다 — `v1.4.199-samsungds`·`v1.4.200-samsungds`의 `skill-guides/`에 `.orca/artifacts`가 0건이고 `config/fork-feature-ledger.json`에도 등재돼 있지 않다. 그래서 `Working files` 절은 지금도 유일하게 실제로 걸리는 경로다. ⓔ 5-3번의 삭제 조건 중 "사내 빌드가 `52eeafc5a0`을 포함"은 `v1.4.200-samsungds`에서 충족됐다(`git merge-base --is-ancestor` 확인). 남은 조건인 "배포 환경에서 자동 분할 ON일 때 소유권이 `owned`로 남는 것"은 편집 머신에서 확인할 수 없으므로 문단을 유지한다.
- 상류 재확인(2026-09-15, v1.4.203 릴리스): 확인 경로는 위와 같은 포크 체크아웃의 `upstream` 태그 `v1.4.203`(= `776e424e76`)과 사내 태그 `v1.4.201-samsungds`(= `6199aa8f5a`, 상류 v1.4.201 머지 포함)다. **삭제 조건에 걸린 커스터마이징은 없다.** ⓐ v1.4.202 → v1.4.203은 grok 훅 수정 4건뿐이고 `skill-guides/`는 v1.4.201 → v1.4.203, 그리고 상류 main `775a9326`까지 변경이 없다. v1.4.201 → v1.4.203에서 바뀐 오케스트레이션 소스는 native-chat 구조화 메일박스 포인터(`8999a00281`)와 아카이브 크기 스캔 조기 종료(`2411491c2f`)로, 커스터마이징이 기대는 제목 파생·릴리스 보존·프리앰블·CLI 스펙 파일은 그대로다. ⓑ v1.4.203 가이드에도 `task-title`·`legacy_ambiguous`·`.orca/artifacts`·`dispatch-spec`은 0건이다(`display-name`·`opencode` 각 1건은 `worktree set --display-name`과 그룹 주소 목록으로, v1.4.201과 같다). 커스터마이징이 쓰는 `--task-title`·`--display-name`·`--report-path`·`--files-modified`·`--timeout-ms`와 `terminalResource`·`ownershipState`·`legacy_ambiguous`는 `src/`에 그대로 있다. ⓒ opencode 절이 기대는 포크 전용 코드(`promptDeliveryMode`, `waitForAgentComposerReady`, `worker-pane-main`, `terminal-typed-user-input`, `assertAgentPromptRescuedIfStalled`)는 상류 v1.4.203에 0건이고, `v1.4.201-samsungds`와 `enterprise/samsungds` HEAD에는 남아 있다. ⓓ 포크 가이드(`v1.4.201-samsungds`)에도 `.orca/artifacts`·`Work Artifacts`는 0건이라 `Working files` 절은 여전히 유일한 경로다. ⓔ `v1.4.201-samsungds`도 `52eeafc5a0`을 포함한다. 5-3번의 남은 조건은 배포 환경 확인이 필요해 그대로 둔다. 4번이 인용한 ponytail은 `e3ba2aa6f1e6`(v4.10.0)으로 올렸다 — Cursor 훅이 추가됐을 뿐 `skills/`는 같고, `SessionStart` 훅의 statusline 제안 주입(`hooks/ponytail-activate.js`)도 Claude Code 세션에 그대로 남아 있어 4번의 근거가 유지된다.
- 상류 재확인(2026-09-16, v1.4.204 릴리스): 확인 경로는 위와 같은 포크 체크아웃의 `upstream` 태그 `v1.4.204`(= `4f7baefc4f`)·상류 main `78609330`과 사내 태그 `v1.4.203-samsungds`(= `enterprise/samsungds` HEAD `fa2b52ee67`, 상류 v1.4.203 머지 포함)의 `skill-guides/`·`src/`다. **삭제 조건에 걸린 커스터마이징은 없다.** ⓐ v1.4.203 → v1.4.204는 53커밋이지만 `skill-guides/`는 변경이 0이고(상류 main `78609330`까지 동일), 상류 `skills/` stub 8종도 `775a9326`과 바이트 단위로 같다. 이 구간에서 커스터마이징이 기대는 파일 중 유일하게 바뀐 것은 `src/cli/specs/orchestration.ts`의 `check` notes 2줄로(`3ab2a1b91c`, delivery 적격성을 messages에서 파생), `--types`가 `--wait` 없는 소비형 check에는 영향이 없다는 것과 Delivery 재생이 `--ack` 외에 "모든 메시지가 read로 표시"로도 끝난다는 설명이다 — 이 스킬의 커스터마이징은 `check`의 `--types`·`--ack`를 쓰지 않으므로 겹치지 않는다. 제목 파생(`orchestration-task-display.ts`)·릴리스 보존(`worker-terminal-release.ts`)·프리앰블(`preamble.ts`)은 v1.4.204에서 변경이 없다. ⓑ v1.4.204 가이드에도 `task-title`·`legacy_ambiguous`·`.orca/artifacts`·`dispatch-spec`은 0건이다(`display-name`·`opencode` 각 1건은 v1.4.201·v1.4.203과 같은 `worktree set --display-name`과 그룹 주소 목록). 커스터마이징이 쓰는 `--task-title`·`--display-name`·`--report-path`·`--files-modified`·`--timeout-ms`와 `terminalResource`·`ownershipState`·`legacy_ambiguous`는 `src/`에 그대로 있다. ⓒ opencode 절이 기대는 포크 전용 코드(`promptDeliveryMode`, `waitForAgentComposerReady`, `worker-pane-main`, `terminal-typed-user-input`, `assertAgentPromptRescuedIfStalled`)는 상류 v1.4.204에 0건이고, `enterprise/samsungds` HEAD에는 5종 모두 남아 있다. ⓓ 포크 가이드(`v1.4.203-samsungds`)에도 `.orca/artifacts`·`Work Artifacts`는 0건이라 `Working files` 절은 여전히 유일한 경로다. ⓔ 포크는 `v1.4.201-samsungds`에서 `v1.4.203-samsungds`로 올라왔고 `52eeafc5a0`을 여전히 포함한다. 5-3번의 남은 조건(배포 환경에서 자동 분할 ON일 때 소유권이 `owned`로 남는 것)은 편집 머신에서 확인할 수 없어 문단을 그대로 둔다. ⓕ 4번이 인용한 ponytail(`e3ba2aa6f1e6`)과 나머지 두 상류(`b36e0829c6d0`, `2c606141936f`)는 저장소 HEAD가 기록된 커밋 그대로다.
- 상류 재확인(2026-09-17, v1.4.205 릴리스): 확인 경로는 위와 같은 포크 체크아웃의 `upstream` 태그 `v1.4.205`(= `11aba8bdc5`)·상류 main `0d23ea6e`와 사내 태그 `v1.4.204-samsungds`(= `origin/enterprise/samsungds` `abf11380ce`, 상류 v1.4.204 머지 포함)의 `skill-guides/`·`skills/`·`src/`다. **삭제 조건에 걸린 커스터마이징은 없다.** ⓐ 이번에는 상류 stub이 바뀌었다. `12d744f2`(#21069, "폴더 만들기" 같은 파일·셸 작업이 `computer-use`로 라우팅되던 문제)가 이 스킬의 `description`에서 Computer Use·Playwright·내장 브라우저 라우팅 문장을 통째로 뺐다(866자 → 524자). 본문은 그대로이고, 이 스킬의 커스터마이징은 description을 쓰지도 인용하지도 않으므로 상류 값을 그대로 받았다. 같은 커밋이 `orca-cli`의 Playwright·CDP 문장을 빼고 `computer-use`를 "프로그램 경로로 안 될 때만"으로 좁혔다 — `computer-use`는 이 배포판이 싣지 않는다. 이 변경은 v1.4.205 태그 **이후** main에 들어갔다(`git merge-base --is-ancestor` 확인). v1.4.205의 stub 8종과 `skill-guides/`는 `78609330`과 같다. 그래서 v1.4.205 앱이 설치하는 stub과 `skills get`이 서비스하는 가이드에는 옛 description이 남아 있다. ⓑ v1.4.204 → v1.4.205(73커밋)에서 오케스트레이션 소스 변경은 구조화 메일박스 전달(`49fba59925`, `955051ded0`)·그룹 주소 주석·`worktree rm --allow-failed-archive-hook`(`f7b2736d6d`)이다. v1.4.205 → main에서는 `--ack`에 message id를 넘겼을 때의 오류 문구(`e6a41081a3`)와 `orca search`(`3631a1e77f`, `orca-cli` 가이드에 `Agent Session Search` 절 추가)다. 커스터마이징은 이 중 어느 것도 쓰지 않는다. 제목 파생(`orchestration-task-display.ts`)·릴리스 보존(`worker-terminal-release.ts`)·프리앰블(`preamble.ts`)·CLI 스펙(`src/cli/specs/orchestration.ts`)은 v1.4.204 → main `0d23ea6e` 사이에 변경이 없다. ⓒ v1.4.205·main 가이드에도 `task-title`·`legacy_ambiguous`·`.orca/artifacts`·`dispatch-spec`·`Work Artifacts`는 0건이다. `display-name`·`opencode` 각 1건은 이전과 같은 `worktree set --display-name`과 그룹 주소 목록이다. `report-path` 2건은 "실제 값이 있을 때만 붙인다"는 문장뿐이라 위치 규약이 아니다. 커스터마이징이 쓰는 `--task-title`·`--display-name`·`--report-path`·`--files-modified`·`--timeout-ms`와 `terminalResource`·`ownershipState`·`legacy_ambiguous`는 v1.4.205·main·`v1.4.204-samsungds`의 `src/`에 그대로 있다. ⓓ opencode 절이 기대는 포크 전용 코드(`promptDeliveryMode`, `waitForAgentComposerReady`, `worker-pane-main`, `terminal-typed-user-input`, `assertAgentPromptRescuedIfStalled`)는 상류 v1.4.205·main에 0건이다. `v1.4.204-samsungds`에는 5종 모두 남아 있고, `v1.4.203-samsungds` → `v1.4.204-samsungds`에서 그 경로의 파일(`worker/` RPC, `tui-agent-config.ts`, `agent-prompt-submit-evidence.ts`, `terminal-typed-user-input.ts`, `preamble.ts`)은 변경이 없다. 포크는 아직 v1.4.205를 태그하지 않았다. ⓔ 포크 가이드(`v1.4.204-samsungds`)에도 `.orca/artifacts`·`Work Artifacts`는 0건이라 `Working files` 절은 여전히 유일한 경로다. `v1.4.204-samsungds`도 `52eeafc5a0`을 포함한다. 5-3번의 남은 조건은 배포 환경 확인이 필요해 그대로 둔다. ⓕ ponytail(`e3ba2aa6f1e6`)·superpowers(`b36e0829c6d0`)·karpathy(`2c606141936f`)는 저장소 HEAD가 기록된 커밋 그대로다.
- 상류 재확인(2026-09-21, v1.4.206 릴리스): 확인 경로는 포크 체크아웃 `/Users/daegun/Workspace/orca`의 `upstream` 태그 `v1.4.206`(= `c62eca3988`)·상류 main `76d87604`와 `enterprise/samsungds` HEAD(`9d42a86f72`, `git describe` = `v1.4.206-558-g9d42a86f72`)의 `skill-guides/`·`skills/`·`src/`다. **삭제 조건에 걸린 커스터마이징은 없다.** ⓐ 이번에는 상류 stub이 그대로다 — `0d23ea6e` → `76d87604`의 297커밋 중 `skills/`를 건드린 것이 0건이라 이 스킬 파일은 바이트 단위로 같다. 대신 지난번 적어 둔 description 변경이 릴리스에 담겼다: v1.4.206 태그의 `orchestration` stub 트리 해시가 `0d23ea6e`의 것과 같다. **릴리스 태그는 릴리스 브랜치라 `merge-base --is-ancestor`로는 판정할 수 없다** — `12d744f2`를 v1.4.206의 조상이 아니라고 답한다. 릴리스에 담겼는지는 `git rev-parse v1.4.206:skills/orchestration`으로 트리 해시를 대조해 본다. 즉 v1.4.206 앱부터는 새 description을 설치한다. ⓑ v1.4.205 → v1.4.206에서 서비스 가이드가 바뀐 곳은 한 곳이다 — `3336933cc8`(#21523)이 `worker-list`를 newest-first로 뒤집고, 100행에서 잘리면 리시트에 `warnings`를 붙이며, `skill-guides/orchestration.md`와 `references/recovery-and-cleanup.md`에 "newest first, `page.nextCursor`를 `--cursor`로 따라가라"를 적었다. **이 스킬의 커스터마이징과는 겹치지 않는다** — `Coordinator field notes`의 `legacy_ambiguous` 문단은 행의 정렬 순서나 페이지 경계를 전제하지 않고(이 파일에 `newest`·`oldest`·`nextCursor`·`hasMore` 0건), 스윕 범위를 Run으로 한정하라는 문장은 새 규약과 같은 방향이다. 가이드는 여전히 `legacy_ambiguous`를 한 번도 다루지 않으므로 추가 3건의 삭제 조건은 발동하지 않는다. ⓒ v1.4.205 → v1.4.206(401커밋)의 오케스트레이션 소스 변경은 위 `worker-list` 정렬, 실패한 Claude 백그라운드 태스크에 타입 행을 주는 것(`7a1f55c52a`), `--ack`에 message id를 넘겼을 때의 오류 문구(`e6a41081a3`) 셋이다. 커스터마이징은 어느 것도 쓰지 않는다. 제목 파생(`orchestration-task-display.ts`)·릴리스 보존(`worker-terminal-release.ts`)·프리앰블(`preamble.ts`)·CLI 스펙(`src/cli/specs/orchestration.ts`)은 이 구간에서 변경이 0건이다. ⓓ v1.4.206 가이드에도 `task-title`·`legacy_ambiguous`·`.orca/artifacts`·`dispatch-spec`·`Work Artifacts`는 0건이다. `display-name` 1건은 `worktree set --display-name`, `report-path` 2건은 "실제 값이 있을 때만 붙인다"는 문장뿐이라 위치 규약이 아니다. 커스터마이징이 쓰는 `--task-title`·`--display-name`·`--report-path`·`--files-modified`·`--timeout-ms`와 `terminalResource`·`ownershipState`·`legacy_ambiguous`는 v1.4.206의 `src/`에 그대로 있다. ⓔ opencode 절이 기대는 포크 전용 코드(`promptDeliveryMode`, `waitForAgentComposerReady`, `worker-pane-main`, `terminal-typed-user-input`, `assertAgentPromptRescuedIfStalled`)는 상류 v1.4.206에 0건이고, `enterprise/samsungds` HEAD에는 5종 모두 남아 있다. **포크는 v1.4.206을 이미 머지했지만 아직 사내 태그를 찍지 않았다** — 최신 사내 태그는 `v1.4.205-samsungds`다. ⓕ 포크 가이드에도 `.orca/artifacts`·`Work Artifacts`는 0건이라 `Working files` 절은 여전히 유일한 경로다. `52eeafc5a0`은 `enterprise/samsungds` HEAD와 `v1.4.205-samsungds` 모두에 포함돼 있다. 5-3번의 남은 조건(배포 환경에서 자동 분할 ON일 때 소유권이 `owned`로 남는 것)은 편집 머신에서 확인할 수 없어 문단을 그대로 둔다. ⓖ 4번이 인용한 ponytail(`e3ba2aa6f1e6`)과 karpathy(`2c606141936f`)는 저장소 HEAD가 기록된 커밋 그대로다. superpowers는 `b36e0829c6d0` → `5bf4e78011`(v6.4.1)로 올랐고 `test-driven-development`와 `systematic-debugging`이 바뀌었다 — 각 스킬의 출처절에 적었고, 라우팅 표와 규약 번호에는 영향이 없다.
- 상류 재확인(2026-09-23, v1.4.209 릴리스): 확인 경로는 상류 클론의 태그 `v1.4.207`(= `6238fd6d4d`)·`v1.4.209`(= `e4d8a9dbc2`)·상류 main `dac82f61`과 포크 체크아웃 `/Users/daegun/Workspace/orca`의 `origin/enterprise/samsungds`(`b243c93c2a`, `git describe` = `v1.4.207-samsungds`)의 `skill-guides/`·`skills/`·`src/`다. **삭제 조건에 걸린 커스터마이징은 없다.** ⓐ v1.4.207·v1.4.209 태그의 `skills/`·`skill-guides/` 트리 해시는 `76d87604`와 같다 — 이 두 릴리스가 설치하는 stub과 서비스하는 가이드는 v1.4.206과 바이트 단위로 같다. ⓑ 상류 main은 그 뒤로 이 stub을 한 번 바꿨다: `9af6a3d798`(#22341)이 `Load the version-matched guide` 절 끝 문단에 "`runtime_access_denied`로 실패하면 샌드박스가 연결을 막은 것이니 권한을 올려 재실행하고, `ORCA open`이나 Orca 재시작은 하지 말라"는 문장을 끼웠다. 상류 원문이라 그대로 받았다(frontmatter는 변경 없음, description 524자 그대로). 이 오류 코드는 v1.4.209와 `enterprise/samsungds`의 `src/`에 아직 0건이고 상류 main의 `src/cli/runtime/runtime-access-denied.ts`에서 처음 생긴다 — 그 전 빌드에서는 조건절이 걸리지 않을 뿐 해가 없다. 커스터마이징 절에는 `ORCA open`·Orca 재시작·`--model`·`--effort`를 쓰는 곳이 없어 겹치지 않는다. 머지 충돌은 이 문단이 바로 뒤의 `Working files` 절과 붙어 있어서 났고, 상류 문단을 받고 추가 절을 그대로 둬 풀었다. 머지 뒤 앞 66줄이 `dac82f61`의 stub과 바이트 단위로 같다. ⓒ 상류 main의 가이드는 `references/coordinator-loop.md`만 두 번 바뀌었다 — `eb92222e7f`(#21705)·`52a1e2875b`(#22383)가 `--model`을 받는 에이전트에 Antigravity·Muse를 더하고 "opencode 등 나머지는 `--model`을 거절하므로 자기 설정의 모델을 쓴다"고 적었다. opencode 절은 `--model`을 쓰지 않으므로 영향이 없다. ⓓ v1.4.206 → v1.4.209(111커밋)에서 `src/main/runtime/orchestration`·`src/cli/specs`·`src/shared/orchestration-task-display.ts` 변경은 0건이다. ⓔ v1.4.209·상류 main·`v1.4.207-samsungds` 가이드 모두 `task-title`·`legacy_ambiguous`·`.orca/artifacts`·`dispatch-spec`·`Work Artifacts`는 0건이라 `Working files`와 `Coordinator field notes`는 여전히 필요하다. 포크 가이드는 `v1.4.206-samsungds` 이후 변경이 없고, `Project Rule Ledger` 절도 그대로라 추가 6건의 전제가 유지된다. ⓕ opencode 절이 기대는 포크 전용 코드(`promptDeliveryMode`, `waitForAgentComposerReady`, `worker-pane-main`, `terminal-typed-user-input`, `assertAgentPromptRescuedIfStalled`)는 상류 v1.4.209에 0건이고 `enterprise/samsungds`에는 5종 모두 있다. 포크는 상류 v1.4.207까지 머지해 `v1.4.207-samsungds`를 찍었고 v1.4.209는 아직 머지하지 않았다. ⓖ superpowers(`5bf4e78011`)·ponytail(`e3ba2aa6f1e6`)·karpathy(`2c606141936f`)는 저장소 HEAD가 기록된 커밋 그대로다.
...
- 추가 4건: 규약 1-2번과 라우팅 표의 `ponytail` 행. 해법의 크기를 정하는 사다리를 spec에 싣는다. 상류 ponytail(`DietrichGebert/ponytail` `e3ba2aa6f1e6`)은 훅과 opencode 플러그인으로 매 턴 규칙 전문(~1,300 토큰)을 주입하는 경로도 제공하지만, 이 배포판은 쓰지 않는다 - 워커 호스트마다 설정이 필요하고, Claude Code용 `SessionStart` 훅이 statusline 설정을 제안하는 지시를 세션에 주입해 무인 워커의 작업을 흐트러뜨린다. 대신 사다리 본문만 규약에 인라인해 워커 종류와 설치 상태에 무관하게 걸리도록 했다. 상류 본문과 충돌하는 두 지점(테스트 생략 허용, 설명 3줄 상한)은 1-2번 안에서 2번과 6번이 이긴다고 명시했다.
