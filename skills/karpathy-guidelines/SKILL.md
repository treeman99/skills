...
**공유 파일은 가정으로 처리하지 않는다.** 빌드 설정, 의존성 매니페스트, 공통 설정처럼 다른 태스크도 건드릴 파일을 고쳐야 하는 상황이면, "가정을 명시하고 진행"에 해당하지 않는다. 병렬 워커가 각자 그렇게 판단하면 같은 파일을 동시에 고쳐 서로의 작업을 덮는다. 이때는 진행하지 말고 코디네이터에게 묻는다:

```bash
orca orchestration ask --question "usagectl에 --json을 넣으려면 Package.swift를 고쳐야 합니다. 다른 워커도 이 파일을 건드리나요? 제가 고쳐도 될지, 아니면 별도 태스크로 뺄지 알려주세요." \
  --options "내가 수정,별도 태스크로 분리" --timeout-ms 600000 --json
```

코디네이터는 어느 워커에게 몰아줄지, 순서를 어떻게 둘지 알고 있다. 워커는 모른다.
