...
### 2026-09-22 — 피드백 루프가 새 프로젝트에서도 시작되게 했다

**증상.** 워커 결과를 지적해도 다음 run의 워커가 같은 실수를 되풀이했다. 지적이 어디에도
남지 않았다.

**원인.** 피드백 루프는 사내 Orca 가이드의 규칙 원장이 담당하는데, 원장은
`.claude/harness/` 폴더가 있는 프로젝트에서만 동작하고 가이드가 그 폴더를 요청 없이 만들지
못하게 한다. 원장 파일이 커밋된 곳은 Orca 포크 저장소뿐이라, 다른 프로젝트에서는 루프가 한
번도 시작되지 않았다. 여기에 결함이 둘 더 있었다.

- 이 번들의 작업 파일 규칙("작업 파일은 전부 `.orca/artifacts/`로")에 원장 예외가 없었다.
  포크 가이드의 옛 규칙에는 예외가 있었지만, 상류 v1.4.199 머지에서 그 절이 통째로 빠졌다.
- opencode 워커는 `--spec`이 한 줄 포인터라, 원장이 "`--spec`에 덧붙이라"는 규칙을 넣을
  자리가 없었다.
...
| 항목 | 내용 |
|---|---|
| 원장 시작 | 원장이 없는 프로젝트에서 첫 지적이 나오면 코디네이터가 원장을 만들지 한 번 묻는다. "피드백 루프 켜줘"라고 하면 묻지 않고 만든다 |
| 템플릿 | `orchestration/harness-template/`에 `rules.md`·`candidates.md`·`retired.md` 빈 파일을 싣는다. 코디네이터는 형식을 지어내지 않고 이것을 복사한다 |
| 작업 파일 예외 | `.claude/harness/`는 작업 파일이 아니라고 명시했다 |
| spec 순서 | 태스크 본문 → `[PROJECT RULES]` → QUALITY CONTRACT. opencode는 세 부분을 모두 스펙 파일에 넣는다 |
| 문서 | 아래 [피드백 루프](#피드백-루프--규칙-원장) 절과 [동작 설명서](docs/how-it-works.md#피드백-루프-규칙-원장)의 흐름도·실패 모드 |

**할 일.** `orchestration` 폴더를 [PowerShell 스크립트](#powershell로-한-번에)로 통째로 다시
복사한다(템플릿 폴더가 그 안에 있다). 그다음 에이전트를 재시작하고,
[확인 절차](#동작하는지-확인하는-법-사내-pc)를 한 번 밟는다.
...
빠졌고, v1.4.207-samsungds와 상류 v1.4.209·main `dac82f61`에도 없다(2026-09-23 확인).
...
원장이 있는 프로젝트에서 코디네이터는 run마다 아래 순서를 밟는다. 사용자가 할 일은 지적하기와
승격 여부에 답하기 두 가지뿐이다.

| 순서 | 시점 | 코디네이터가 하는 일 | 사용자가 하는 일 | 바뀌는 파일 |
|---|---|---|---|---|
| 1 | run 시작, 첫 `task-create` 전 | `rules.md`를 한 번 읽는다. 지난 run에서 정리하지 못한 후보가 있으면 먼저 묻는다 | 미결 후보의 승격·기각에 답한다 | (답에 따라) `rules.md`, `retired.md` |
| 2 | 태스크를 만들 때마다 | 그 태스크의 요청이나 건드릴 파일에 `scope`가 맞는 블록만 골라 spec에 `[PROJECT RULES]`로 넣는다. 최대 5개, 적중 수가 많은 순이다. 넣은 블록은 적중을 1 올린다 | — | `rules.md`(적중 수) |
| 3 | 워커 실행 중 | 평소대로 감독한다. 워커는 spec에 실린 규칙을 따른다 | 실행 중 프로젝트 요구를 더 줄 수 있다 → 5번과 같이 기록 | — |
| 4 | `worker_done` 뒤 | diff나 검증 명령으로 독립 확인한 뒤 보고한다 | 결과가 틀렸으면 지적한다 | — |
| 5 | 지적이나 새 요구를 받았을 때 | `candidates.md`에 한 건 적고 적었다고 알린다. 요약이 아니라 **다음 워커가 할 일**로 적는다. 이번 결과물에 대한 일회성 취향은 적지 않는다 | — | `candidates.md` |
| 6 | 마지막 워커가 정산된 뒤 | 자격이 있는 후보를 전부 묻는다. 자격은 같은 지적이 두 번 관측됐거나, 사용자가 그 후보를 콕 집어 요청한 것이다 | 승격할지 기각할지 정한다 | `rules.md` 또는 `retired.md` |

세션이 예고 없이 끝나 6번을 건너뛰어도, 다음 run의 1번이 같은 후보를 다시 꺼낸다. 미결
후보를 세 번째 run까지 말없이 끌고 가지는 않는다.

### 세 번의 run으로 보는 예

원장이 없는 프로젝트에서 "API 응답의 날짜를 로컬 시간 문자열로 내보낸다"는 실수가 되풀이되는
경우다.
...
sequenceDiagram
    ...
    participant W as 워커
    participant L as .claude/harness/

    rect rgba(120, 140, 200, 0.14)
    Note over U,L: run 1 — 원장이 아직 없다
    U->>C: 주문 조회 API 만들어줘
    C->>W: 본문 + QUALITY CONTRACT
    W->>C: worker_done
    C->>U: 독립 확인 후 보고
    U->>C: 날짜는 ISO 8601 UTC로 내보내야 해
    C->>U: 원장이 없어 이 지적은 다음 워커에게 가지 않는다. 만들까?
    U->>C: 만들어
    C->>L: 템플릿 3개 복사 + C-001 기록 (관측 1회)
    C->>W: 지적을 반영한 후속 디스패치
    Note over C: 정산 뒤 심사 — C-001은 관측 1회라 아직 묻지 않는다
    end

    rect rgba(200, 160, 100, 0.14)
    Note over U,L: run 2 — 후보는 워커에게 가지 않는다
    C->>L: rules.md 읽기 (규칙 없음)
    C->>W: 본문 + QUALITY CONTRACT
    W->>C: worker_done
    U->>C: 날짜 또 로컬 시간이네
    C->>L: C-001 관측 2회
    Note over C: 정산 뒤 심사 — 자격이 생겼다
    C->>U: C-001을 규칙으로 올릴까?
    U->>C: 올려
    C->>L: rules.md에 R-001 (적중 0)
    end

    rect rgba(110, 180, 130, 0.14)
    Note over U,L: run 3 — 규칙이 워커에게 간다
    C->>L: rules.md 읽기
    C->>W: 본문 + [PROJECT RULES] R-001 + QUALITY CONTRACT
    C->>L: R-001 적중 +1
    W->>C: worker_done (UTC로 직렬화, 형식 테스트 포함)
    end
```

run 2에서 같은 실수가 한 번 더 나는 것은 결함이 아니라 설계다. `candidates.md`는 어떤 워커의
컨텍스트에도 들어가지 않는다. 한 번 나온 지적을 곧바로 모든 워커에게 퍼뜨리지 않고, 두 번
확인된 것만 사람이 승인해 올린다. 한 번 만에 올리고 싶으면 그 후보를 콕 집어 "C-001 지금
규칙으로 올려"라고 요청하면 된다.

run 2에서 승격하기 직전의 `candidates.md` 항목과, run 3이 끝난 뒤의 `rules.md` 블록은 이런
모양이다.

```text
### C-001 · scope: api · 관측 2회 · 2026-09-22
API 응답에 날짜·시간을 넣을 때는 로컬 시간 문자열이 아니라 ISO 8601 UTC
(2026-09-22T03:00:00Z)로 직렬화하고, 형식을 단언하는 테스트를 같은 태스크에 남겨라.
출처: task_1a2b3c worker_done 직후 사용자 지적
```

```text
### R-001 · scope: api

**언제** API 응답에 날짜·시간 필드를 추가하거나 바꿀 때
**규칙** ISO 8601 UTC 문자열로 직렬화하고, 형식을 단언하는 테스트를 같은 태스크에 남긴다.
**이유** 로컬 시간 문자열을 두 run 연속 내보내 사용자가 두 번 지적했다.
**출처** C-001 · task_1a2b3c, task_4d5e6f · **적중** 1
```

### 워커 spec에 실리는 모양

규칙은 원장 파일 전체가 아니라 **그 태스크에 맞는 블록만** 실린다. 맞지 않는 규칙은 워커가
헤쳐 가야 할 잡음이고 태스크 본문을 밀어낸다. 순서는 항상 아래와 같고 규약 블록이 맨 끝이다.

```text
<태스크 본문 — Target / Change / Constraints / Ownership / Observable acceptance>
...
심사에서 코디네이터가 제안하지 않거나 다른 길로 돌리는 경우다. 원장을 작게 유지하는 장치라
규칙이 적게 쌓이는 것이 정상이다.

| 후보 | 처리 |
|---|---|
| 프로젝트의 lint·typecheck·테스트가 이미 잡는 실수 | 기각. 워커 컨텍스트만 먹는 중복이다 |
| 기존 블록과 `scope`가 겹친다 | 새 블록 대신 병합안을 제시한다 |
| 이 머신에서만 참인 사실(로컬 경로, 개인 설정) | 커밋하지 않는다 |
| `rules.md`가 블록 12개·200줄을 넘게 된다 | 추가와 함께 폐기 1건을 제안한다. 한 번도 주입되지 않은 블록이 첫 후보이고, 내린 블록은 `retired.md`로 간다 |

코디네이터는 원장 밖에 배운 것을 적지 않는다. `CLAUDE.md`나 `AGENTS.md`에 대신 적는 것도
가이드가 금지한다.

### 원장이 없는 프로젝트에서 시작하기 — 이 번들이 더한 부분
...
번들은 그 빈틈만 메운다. 위 예의 run 1이 그 경로다.
...
경로에 들어가므로, 상류를 반영할 때 description 길이를 함께 본다. 현재(`dac82f61`)는
...
| 스킬 | 상류 | 커밋 | 라이선스 |
|---|---|---|---|
| `orchestration` | `stablyai/orca` | `dac82f61` | 상류 저장소 라이선스 |
| `orca-cli` | `stablyai/orca` | `dac82f61` | 상류 저장소 라이선스 |
