...
### 작업 파일은 `.orca/artifacts/<task_id>/` 안에만

계획, 중간 분석, 리뷰 노트, 장문 리포트는 태스크가 산출물로 요구받은 것이 아니라 **일을
하려고 만든 부산물**이다. 규칙이 없으면 워커마다 제 자리를 정해서, 네 태스크짜리 런 하나가
저장소 루트에 `PLAN-auth.md`, 읽던 코드 옆에 `notes.md`, 그리고 코디네이터가 끝내 못 본
파일 둘을 남긴다. 코드만 담겨야 할 diff에서 사용자가 그것들을 골라내게 된다.

규약 1-1번이 이것을 한 곳으로 모은다.

- **`.orca/`는 이미 Orca의 워크스페이스 네임스페이스다** — `.orca/drops`, `.orca/templates`,
  `.orca/browser-downloads`, `.orca/issue-command`. ignore 항목 하나면 모든 태스크의 발판이
  덮이고, **작업 파일은 추적되지 않은 채** `git status`와 diff에서 통째로 빠진다.
- **다만 ignore 항목은 저장소마다 한 번 확인해야 한다.** Orca가 `.gitignore`에 `.orca`를 써
  넣기는 하지만 앱이 그 디렉터리에 직접 뭔가를 만들 때만이라, 그 경로를 밟은 적 없는 저장소에는
  항목이 없다. 없으면 규칙은 어질러진 것을 옮기기만 하고 `git status`에서 치우지는 못한다.
  ```bash
  grep -qxF .orca .gitignore || echo .orca >> .gitignore
  ```
- **폴더 이름은 설명이 아니라 task id다.** 코디네이터와 `worker_done` 페이로드와
  `dispatch-show`가 이미 공유하는 유일한 식별자라, 남은 파일이 어느 태스크의 것인지 항상
  되짚을 수 있다. 폴더 안에서는 이름을 자유롭게 쓴다.
- **산출물은 예외다.** 태스크가 만들라고 한 소스·테스트·문서는 프로젝트가 두는 자리에 둔다.
  이 규칙은 일을 하려고 만든 발판만 대상으로 한다.
- **`--report-path`는 그 안을 가리킨다.** 코디네이터가 파일을 찾아 헤매지 않고 장문 보고서를
  여는 것이 그 플래그의 존재 이유다.

**코디네이터가 주입하지 않으면 걸리지 않는다.** 디스패치 프리앰블은 이 규칙을 싣지 않는다.
규약 블록 안에 넣어 둔 이유가 이것이다 — 붙일 블록이 둘이면 하나는 빠진다.

`orca skills get orchestration`이 서비스하는 가이드에는 이 규약이 없다. 상류 소스에 같은
취지의 초안이 있지만 아직 커밋되지 않았고, 릴리스되면 이 절은 지우고 가이드를 따른다.
